...
//char waitForNextHeliumChar() {
//	while(!csk_uart1_count()) OS_Delay(1);
//	char next = csk_uart1_getchar();
//		char tmp[32];
//	if (next == 'H') {
//		strcpy(tmp, "YES:"); sprintf(tmp, "%s%c", tmp, next); HeTrans255Str(tmp);
//	} else {
//		strcpy(tmp, "No :"); sprintf(tmp, "%s%c", tmp, next); HeTrans255Str(tmp);
//	}
//	return(next);
//}
 ...
 */
int readAllFromHelium(char* heBuffer, int readPos, int parsePos, int bufferLength) {
//	readPos = incrementCarefully(readPos, parsePos, bufferLength);
	int startRead = readPos;
	while ((csk_uart1_count()) && (readPos != -1)) {
//		int ct = csk_uart1_count();
		char pt = 'Z';
//		if (ct > 9) { pt = ct + '0'; } else pt = 'A';
		heBuffer[readPos] = pt; // csk_uart1_getchar();
		// Increment until we reach the spot before parsePos
//		if (heBuffer[readPos] != 0x00) {
			readPos = incrementCarefully(readPos, parsePos-1, bufferLength);
//		} else {
//			OS_Delay(1);
//		}
		if (readPos - startRead > 40) HeTrans255(heBuffer, readPos);
	}
	if (readPos == -1) {
		if (parsePos == 0) {
...
//	unsigned int size;
	...
	// is at (readPos-1)
	int readPos = 0;
//	clearBuffer(heBuffer, BUFFER_LEN);
//	heBuffer[0] = 'Z';
//	heBuffer[1] = 'o';
//	heBuffer[2] = 'o';
//	heBuffer[3] = 0;
...
//		readPos = readAllFromHelium(heBuffer, readPos, parsePos, BUFFER_LEN);
